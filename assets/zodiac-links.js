...
(function(){
  ...
  function relationScore(birth,today){
    const a=elementIndex[birth.element],b=elementIndex[today.element];
    if(a===b)return 4;
    if((a+1)%5===b)return 5;
    if((b+1)%5===a)return 3;
    if((a+2)%5===b)return 1;
  ...
  function calcFortune(year,month,day){
    ...
    // 생년월일과 오늘 날짜만으로 계산하는 결정론적 행운 정보입니다. 랜덤값은 사용하지 않습니다.
    const seed=Math.abs(julianDay(year,month,day)*31+julianDay(today.index+2000,1,1)*7+yearRelation*17+dayRelation*23+branchRelation*29);
    const colors=['아이보리','연두','하늘색','베이지','금빛','청록','라벤더','연분홍'];
    const directions=['동쪽','서쪽','남쪽','북쪽','남동쪽','남서쪽','북동쪽','북서쪽'];
    const times=['07:00~09:00','09:00~11:00','11:00~13:00','13:00~15:00','15:00~17:00','17:00~19:00','19:00~21:00','21:00~23:00'];
    const items=['지갑','손목시계','노트','펜','키링','머그컵','이어폰','카드지갑'];
    const lucky={
      color:colors[seed%colors.length],
      number:((seed%45)+1),
      time:times[(seed>>2)%times.length],
      direction:directions[(seed>>4)%directions.length],
      item:items[(seed>>6)%items.length]
    };

    return {birthYear,birthDay,today,scores,level,relation:relationText[yearRelation],moneyText:moneyText[band(scores.money)],loveText:loveText[band(scores.love)],workText:workText[band(scores.work)],healthText:healthText[band(scores.health)],lucky};
  }
  ...
  function pending(){
    ...
    if(!o)return;
    document.body.classList.add('mira-pre-input-lock');
    ...
    // .four는 .overview 바깥에 있으므로 overview 내부만 찾지 않고 문서 전체에서 초기화합니다.
    document.querySelectorAll('.four .fortune').forEach(card=>{
      card.querySelectorAll('strong,small,.score,[data-mira-fortune]').forEach(el=>{el.textContent='';el.style.setProperty('display','none','important');});
    });
    document.querySelectorAll('.luckrow b').forEach(el=>el.textContent='');
  }
  ...
  function show(year,month,day){
    ...
    document.body.classList.remove('mira-pre-input-lock');
    ...
    // 하단 4개 카드는 overview 바깥에 있어 문서 전체에서 정확히 찾아 표시합니다.
    document.querySelectorAll('.four .fortune').forEach((c,n)=>{
      ...
      if(n>3)return;
      c.querySelectorAll('[data-mira-fortune]').forEach(el=>el.remove());
      const oldStrong=c.querySelector('strong');
      const oldSmall=c.querySelector('small');
      if(oldStrong){oldStrong.textContent=scores[n]+'점';oldStrong.style.removeProperty('display');}
      else{const s=document.createElement('strong');s.dataset.miraFortune='score';s.textContent=scores[n]+'점';c.appendChild(s);}
      if(oldSmall){oldSmall.textContent=texts[n];oldSmall.style.removeProperty('display');}
      else{const p=document.createElement('small');p.dataset.miraFortune='text';p.textContent=texts[n];c.appendChild(p);}
    });

    // 오른쪽 행운 정보 5개도 같은 생년월일 공식에서 함께 계산합니다.
    const luckValues=[f.lucky.color,String(f.lucky.number),f.lucky.time,f.lucky.direction,f.lucky.item];
    document.querySelectorAll('.luckrow b').forEach((el,n)=>{if(n<luckValues.length){el.textContent=luckValues[n];el.style.removeProperty('display');}});
  }
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='mira-final-style';s.textContent=`
      ...
      .mira-date-popup.open{display:block!important}.mira-date-popup .mira-date-title{font-size:12px!important;font-weight:900!important;color:#53655b!important;margin-bottom:9px!important}.mira-date-popup .mira-date-selects{display:grid!important;grid-template-columns:1.3fr 1fr 1fr!important;gap:6px!important}.mira-date-popup select{width:100%!important;height:38px!important;border:1px solid #d3cdbf!important;border-radius:8px!important;background:#fff!important;padding:0 8px!important;font-size:13px!important;color:#34443c!important;cursor:pointer!important}.mira-date-popup .mira-date-actions{display:flex!important;justify-content:flex-end!important;gap:6px!important;margin-top:9px!important}.mira-date-popup button{height:34px!important;padding:0 12px!important;border-radius:8px!important;border:1px solid #d3cdbf!important;background:#fff!important;color:#52645a!important;font-weight:800!important;cursor:pointer!important}.mira-date-popup .mira-date-ok{background:#5c8d71!important;color:#fff!important;border-color:#5c8d71!important}
      .overview.mira-pending .score{display:none!important}.overview.mira-pending .card>*{visibility:hidden!important;opacity:0!important}.overview.mira-pending .card{visibility:visible!important}.overview.mira-pending .four .fortune>*{visibility:hidden!important;opacity:0!important}.overview.mira-pending .four .fortune h3{visibility:hidden!important;opacity:0!important}.overview:not(.mira-ready) .four .fortune>*{visibility:hidden!important;opacity:0!important}.overview.mira-ready .score{display:flex!important}.overview.mira-ready .card>*{visibility:visible!important;opacity:1!important}
      body.mira-pre-input-lock .four .fortune>*{visibility:hidden!important;opacity:0!important}.four .fortune strong,.four .fortune small{visibility:visible!important;opacity:1!important}.four .fortune{min-height:91px!important}
      @media(max-width:760px) and (hover:none) and (pointer:coarse){.tests{display:none!important}.birth-form{grid-template-columns:115px minmax(0,1fr) auto 68px!important;width:100%!important;gap:5px!important}.birth-form .birth-result{width:115px!important;min-width:115px!important;max-width:115px!important;font-size:10px!important;transform:translateX(-12px)!important}.birth-form .mira-date-trigger{grid-column:2!important;font-size:12px!important}.birth-form button[type="submit"]{grid-column:3!important;padding:0 10px!important}.birth-form .birth-reset{grid-column:4!important;width:68px!important;min-width:68px!important;font-size:11px!important;padding:0 6px!important}.mira-date-popup{position:fixed!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%)!important;width:min(330px,calc(100vw - 24px))!important}}
  ...
  function setup(){
    ...
    trigger.type='button';trigger.className='mira-date-trigger';trigger.textContent='생년월일 선택';trigger.setAttribute('aria-label','생년월일 선택');
    ...
    reset.type='button';reset.textContent='초기화';
    ...
    const popup=document.createElement('div');popup.className='mira-date-popup';
    ...
    const yearPlaceholder=document.createElement('option');yearPlaceholder.value='';yearPlaceholder.textContent='년도';yearPlaceholder.disabled=true;yearPlaceholder.selected=true;ys.appendChild(yearPlaceholder);
    ...
    function fillDays(){const y=Number(ys.value),m=Number(ms.value)||1,max=y?new Date(y,m,0).getDate():31,oldDay=Number(ds.value)||1;ds.innerHTML='';for(let d=1;d<=max;d++){const o=document.createElement('option');o.value=d;o.textContent=d+'일';ds.appendChild(o);}ds.value=String(Math.min(oldDay,max));}
    ms.value='1';fillDays();ys.addEventListener('change',fillDays);ms.addEventListener('change',fillDays);
    function openPicker(){const rect=trigger.getBoundingClientRect();popup.classList.add('open');if(window.matchMedia('(max-width:760px) and (hover:none) and (pointer:coarse)').matches)return;popup.style.left=Math.max(12,Math.min(window.innerWidth-342,rect.left))+'px';popup.style.top=(rect.bottom+7)+'px';}
    ...
    popup.querySelector('.mira-date-ok').addEventListener('click',()=>{const y=Number(ys.value),m=Number(ms.value),d=Number(ds.value);if(!y)return;trigger.textContent=`${y}년 ${String(m).padStart(2,'0')}월 ${String(d).padStart(2,'0')}일`;trigger.dataset.date=`${y}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`;r.textContent=y+'년생 · '+animal(y);r.classList.remove('birth-warning');closePicker();});
    ...
    form.addEventListener('submit',e=>{
      ...
      r.classList.remove('birth-warning');r.textContent=y+'년생 · '+animal(y);
    ...
    reset.addEventListener('click',e=>{
      e.preventDefault();e.stopImmediatePropagation();
      trigger.textContent='생년월일 선택';delete trigger.dataset.date;r.textContent='';r.classList.remove('birth-warning');
    ...
    try{
      ...
      if(saved&&/^\d{4}-\d{2}-\d{2}$/.test(saved)){
        ...
        trigger.dataset.date=saved;trigger.textContent=`${y}년 ${String(m).padStart(2,'0')}월 ${String(d).padStart(2,'0')}일`;r.textContent=y+'년생 · '+animal(y);
      }
    }catch(x){}
    // 저장된 생년월일이 있어도 결과는 자동 표시하지 않고, 운세 확인을 눌렀을 때만 표시합니다.
    pending();
  }
