// MIRA 홈 운세 UI 최종 고정
// 기존 패치/인라인 코드와 충돌해도 마지막 상태를 복구합니다.
(function(){
  ...
  function hideTests(){
    if(!home())return;
    document.querySelectorAll('.tests,[href="#tests"],[href="/test/"],[href="/test"]').forEach(el=>el.remove());
  }
  ...
  function result(year,month,day){
    ...
    const seed=year*31+month*17+day*13+new Date().getDate()*7+new Date().getMonth()*11;
    const score=76+(seed%20);
    const main=o.querySelector('.main');
    if(main){
      const b=main.querySelector('.badge');if(b)b.textContent=['좋은 흐름','안정적인 흐름','기회가 오는 흐름','차분한 흐름'][year%4];
      const h=main.querySelector('h3');if(h)h.textContent=['작은 변화가 좋은 흐름을 만들어요.','천천히 움직일수록 결과가 좋아요.','새로운 기회를 놓치지 마세요.','정리와 집중이 행운을 불러요.'][year%4];
      const p=main.querySelector('p');if(p)p.textContent=['눈앞의 일을 하나씩 정리하면 생각보다 수월하게 풀리는 날이에요.','서두르기보다 순서를 정하면 오늘의 운이 안정적으로 이어져요.','평소와 다른 선택 하나가 오늘의 분위기를 바꿔줄 수 있어요.','해야 할 일을 가볍게 정리하면 마음도 한결 편안해져요.'][year%4];
      const s=main.querySelector('.score b');if(s)s.textContent=score;
    }
    const texts=['재물운','애정운','직장운','건강운'];
    o.parentElement?.querySelectorAll('.four .fortune').forEach((c,i)=>{const s=c.querySelector('strong'),p=c.querySelector('small');if(s)s.textContent=['상승','좋음','활기','안정'][i];if(p)p.textContent=['계획적인 소비에 집중하세요.','솔직한 대화가 좋은 분위기를 만들어요.','미뤄둔 일을 하나씩 정리해보세요.','무리하지 말고 충분히 쉬어주세요.'][i];});
  }
  function setup(){
    if(!home())return;
    hideTests();
    ...
    if(!ready){
      const fresh=old.cloneNode(true);old.replaceWith(fresh);ready=true;
    }
    const f=document.getElementById('birthForm');const input=f.querySelector('#birthYear')||f.querySelector('input');if(!input)return;
    ...
    input.type='date';input.min='1900-01-01';input.max=new Date().toISOString().slice(0,10);input.classList.add('mira-home-date');f.insertBefore(r,input);
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='mira-final-style';s.textContent=`
      ...
      @media(max-width:760px) and (hover:none) and (pointer:coarse){.tests{display:none!important}.birth-form{grid-template-columns:72px minmax(0,1fr) auto 60px!important;width:100%!important;gap:5px!important}.birth-form .birth-result{grid-column:1!important;width:72px!important;min-width:72px!important;max-width:72px!important;font-size:10px!important}.birth-form .mira-home-date,.birth-form input{grid-column:2!important}.birth-form button[type="submit"]{grid-column:3!important;padding:0 10px!important}.birth-form .birth-reset{grid-column:4!important;width:60px!important;min-width:60px!important;font-size:11px!important}}
    `;document.head.appendChild(s);
  }
  function boot(){hideTests();setup();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  const observer=new MutationObserver(()=>{if(home()){hideTests();setup();}});
  const start=()=>document.body&&observer.observe(document.body,{childList:true,subtree:true});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  setInterval(()=>{if(home())setup();},1000);
})();
